...
function switchImage(n) {
    ...
    ulBanner.style.marginLeft = value; //计算它最终的margin-left
    //清除之前的active
    var before = document.querySelector(".banner-title .active");
    before.className = "";
    //下面，搞定下方的li的选中效果
    //加入active效果
    var active = document.querySelector(".banner-title ");
...
ulTitle.onmouseover = function(e) {
    ...
    var index = Array.from(ulTitle.children).indexOf(e.target);
...
//开始切换图片（3秒切换一次）
function start() {
    // 管它那么多，先把之前的清除掉，保证只有一个计时器
    clearInterval(timer);
    //切换图片
    timer = setInterval(function() {
        currentIndex++;
        if (currentIndex == 5) { //最大只能是4，到了5，就回到0
            currentIndex = 0;
        }
